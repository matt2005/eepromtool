...
class ecueeprom:
    ...
    def parse(self):
        ...
        if self.size == 1024 or R32 == True:
            ...
            if R32:
                logging.info(
                    "The last 512bytes are padded with 0xFF, this appears to be an R32 bin")
                self.R32 = True
        ...
        if self.data[0x1][0xC] + self.data[0x2][0xC] > 0:
            self.dtc = "%0.2X %0.2X" % (
                self.data[0x1][0xC], self.data[0x2][0xC])
    ...
    def validateChecksum(self):
        try:
            ...
            for pageno in range(0x00, len(self.data)):
                ...
                savedsum = "%0.2X%0.2X" % (
                    self.data[pageno][0x0F], self.data[pageno][0x0E])
            ...
            logging.warn(
                "WARNING: Cannot validate checksum, possible invalid input")
            self.csum = False
    ...
    def writebin(self, outputfile):
        ...
        print("")
        logging.debug("Writing %dkb ECU eeprom bin to: %s" %
    ...
    def printStatus(self):
        ...
        logging.info("- Flash Programming (successful): %d" %
                     self.flashsuccessfulcount)
        logging.info("- Flash Programming (attempts): %d" %
                     self.flashattemptcount)
        logging.info("")
